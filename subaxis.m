...
%
%subaxis()  使用说明
% 用法与subplot很相似，前三个参数与subplot一样
% 后面的设置可以 设置:
%轴间距（Spacing: SpacingVert, SpacingHoriz），
% 轴缩放（ Padding,PaddingRight,PaddingLeft,PaddingTop,PaddingBottom）
%  边距（Margin： MarginRight,MarginLeft,MarginTop,MarginBottom）
% 保持原来的输出： Holdaxis
% 上述设置均可以使用缩略字符表示，例如用SV表示SpacingVert
